...
def page_bigdata():
    ...
    st.markdown(
        "This page separates the public Streamlit demo from the full local "
        "engineering environment. The deployed app shows a reproducible snapshot "
        "of pipeline outputs, while the complete Hadoop/Hive/Spark/PostgreSQL "
        "stack runs locally on macOS with Docker Compose."
    )
    df = load_signals()
    if df.empty:
    ...
    if use_postgres():
        st.success(
            "Local full-stack mode: the dashboard is connected to PostgreSQL and "
            "can be used alongside the Docker services below."
        )
        st.markdown(
        ...
        st.caption("These links work only while the local Docker stack is running.")
    else:
        st.info(
            "Public Streamlit Cloud mode: this app is intentionally running from "
            "exported CSV snapshots because Streamlit Cloud cannot expose this "
            "project's local Docker network, HDFS NameNode, Spark UI, Airflow UI, "
            "FastAPI server, or PostgreSQL container."
        )
        st.subheader("What runs locally on my Mac")
        st.markdown(
            "- **Docker Compose:** PostgreSQL plus local service orchestration  \n"
            "- **HDFS/Hive design:** `hive/*.hql` and `ingestion/load_to_hdfs.py`  \n"
            "- **PySpark processing:** `ingestion/parse_faers.py`, `spark/data_cleaning.py`, `spark/feature_engineering.py`  \n"
            "- **ML and validation:** `ml/train_model.py`, `ml/predictor.py`, `ml/evaluate.py`  \n"
            "- **Local dashboard mode:** reads PostgreSQL tables generated by the pipeline  \n"
            "- **Cloud demo mode:** reads `dashboard/data/*.csv` snapshots exported from the local run"
        )
        st.caption(
            "In short: the public link is the portfolio demo; the full big-data "
            "pipeline is reproducible locally with Docker/PySpark/PostgreSQL."
        )
